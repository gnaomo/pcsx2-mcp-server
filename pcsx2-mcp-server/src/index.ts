...
async function identifyInstance(entry: InstanceEntry): Promise<void> {
  ...
  } catch { /* fall through to Pine */ }
  try {
    if (entry.pine?.isConnected()) {
      const title = await entry.pine.getTitle();
      if (title) {
        entry.label = title;
        try { entry.serial = await entry.pine.getID(); } catch { /* title alone is still useful */ }
      }
    }
  } catch { /* leave label/serial unset */ }
}
...
function resolveInstanceId(query: string): string {
  ...
  const matches = [...instances.values()].filter(e =>
    e.id.toLowerCase().includes(q) ||
    (e.label && e.label.toLowerCase().includes(q)) ||
    (e.serial && e.serial.toLowerCase().includes(q))
  );
  if (matches.length === 1) return matches[0].id;
  if (matches.length === 0) throw new Error(`No instance matching "${query}". Use pcsx2_list_instances to see connected instances (id, and game title/serial if identified).`);
  throw new Error(`"${query}" matches multiple instances: ${matches.map(m => m.id).join(', ')}. Be more specific, or use the exact instance_id.`);
}
...
server.tool('pcsx2_connect',
  'Connect to a PCSX2 instance. Tries DebugServer (21512), then Pine (28011). DebugServer gives FULL access (128-bit regs, expressions, conditional BP, native disasm). Pine gives memory + game info. Multi-instance: pass a distinct instance_id + debug_port/pine_port for each running PCSX2 process (each instance needs its own DebugServerPort configured — see ROADMAP.md Phase 0). The newly-connected instance becomes the active one; use pcsx2_use_instance to switch back to another already-connected instance, and pcsx2_list_instances to see them all.',
  {
    ...
    instance_id: z.string().optional().describe('Name this connection for later pcsx2_use_instance calls. Defaults to a name derived from the ports, e.g. "debug21512".'),
  },
...
// ==========================================================
server.tool('pcsx2_discover_instances',
  ...
  async ({ start_port, count }) => {
    ...
    for (let port = start_port; port < start_port + count; port++) {
      if (alreadyPorts.has(port)) { skipped.push(`${port} (already connected as "${alreadyPorts.get(port)}")`); continue; }
...
// ==========================================================
server.tool('pcsx2_forget_instance',
  ...
  { instance_id: z.string().describe('Exact id, or a substring matching id/game title/serial - same resolution rules as pcsx2_use_instance') },
  async ({ instance_id }) => {
    try {
      const resolved = resolveInstanceId(instance_id);
      ...
      return { content: [{ type: 'text' as const, text: `Forgot instance "${resolved}" (${instances.size} instance(s) remaining).${activeInstanceId === null ? ' No instance is active now.' : ''}` }] };
    } catch (e: any) {
      return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true };
    }
  }
...
server.tool('pcsx2_status', 'Get connection + emulator status.', {},
  async () => {
    const p: string[] = [];
    p.push(`Active instance: ${activeInstanceId ?? '(none)'} — ${instances.size} instance(s) connected total (pcsx2_list_instances for details)`);
...
server.tool('pcsx2_read_memory_multiple', 'Batch memory read: one round-trip for many independent {address, length} reads, mirroring the read_multiple_files pattern. Use this instead of several sequential pcsx2_read_memory calls when checking a scattered set of known addresses (e.g. a watch list of struct fields, or several candidate pointers at once). A bad address in one entry doesn\'t fail the batch - that entry just comes back flagged as not fully valid. Requires DebugServer.',
  ...
  async ({ reads, cpu }) => {
    ...
    try {
      ...
      const lines = results.map(r => `${r.address} (${r.length}B)${r.allValid ? '' : ' [PARTIALLY INVALID]'}: ${r.hex}`);
      return { content: [{ type: 'text' as const, text: lines.join('\n') }] };
    } catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);
...
// ==========================================================
server.tool('pcsx2_read_vu1_registers',
...
// ==========================================================
server.tool('pcsx2_read_vu_micromem', 'Dump VU0/VU1 instruction memory (imem) as a raw hex string. VU0 micro-mem is 4KB (0x1000, offsets 0-0xFFC), VU1 is 16KB (0x4000, offsets 0-0x3FF8). This is raw bytes for tooling/diffing - for a human-readable view of the actual code, use pcsx2_disassemble_vu instead. Requires DebugServer.',
  { vu: z.union([z.literal(0), z.literal(1)]).default(0), address: z.number().min(0).default(0).describe('Byte offset into VU micro-mem'), length: z.number().min(1).max(16384).default(256) },
  async ({ vu, address, length }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
      const { hex, progSize } = await debugServer!.readVUMicroMem(vu, address, length);
      const buf = Buffer.from(hex, 'hex');
      return { content: [{ type: 'text' as const, text: `VU${vu} micro-mem at 0x${address.toString(16)} (prog_size=0x${progSize.toString(16)}):\n\n${hexDump(buf, address)}` }] };
    } catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);
...
// ==========================================================
server.tool('pcsx2_disassemble_vu',
  ...
  { vu: z.union([z.literal(0), z.literal(1)]).default(0), address: z.number().min(0).default(0).describe('Byte offset into VU micro-mem, auto-aligned down to a multiple of 8'), count: z.number().min(1).max(500).default(20) },
  async ({ vu, address, count }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
      ...
      return { content: [{ type: 'text' as const, text: `VU${vu} microcode disassembly (${instrs.length} instructions):\n\n${lines.join('\n')}` }] };
    } catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);
...
// ==========================================================
server.tool('pcsx2_set_vu_breakpoint',
  ...
  { vu: z.union([z.literal(0), z.literal(1)]).default(0), address: z.number().min(0).describe('Byte offset into VU micro-mem, auto-aligned down to a multiple of 8'), description: z.string().optional() },
  async ({ vu, address, description }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
...
server.tool('pcsx2_remove_vu_breakpoint', 'Remove a VU-side address breakpoint.',
  { vu: z.union([z.literal(0), z.literal(1)]).default(0), address: z.number().min(0) },
  async ({ vu, address }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try { await debugServer!.removeVUBreakpoint(vu, address); return { content: [{ type: 'text' as const, text: `VU${vu} breakpoint removed at 0x${address.toString(16)}` }] }; }
    catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);

server.tool('pcsx2_list_vu_breakpoints', 'List VU0/VU1 breakpoints. Omit vu to list both.',
...
server.tool('pcsx2_clear_vu_breakpoints', 'Clear VU0/VU1 breakpoints. Omit vu to clear both.',
  { vu: z.union([z.literal(0), z.literal(1)]).optional() },
  async ({ vu }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try { await debugServer!.clearVUBreakpoints(vu); return { content: [{ type: 'text' as const, text: vu !== undefined ? `VU${vu} breakpoints cleared.` : 'VU0 and VU1 breakpoints cleared.' }] }; }
    catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);
...
// ==========================================================
server.tool('pcsx2_get_gs_context',
  ...
  async () => {
    ...
    try {
      ...
      if (!r.gs_ready) return { content: [{ type: 'text' as const, text: 'GS is not ready yet (no frame rendered since boot).' }] };

      const lines: string[] = [`Active context: ${r.active_context}`, ''];
      const e = r.environment;
      lines.push('--- Environment ---');
      lines.push(`PRIM: type=${e.prim.prim} iip=${e.prim.iip} tme=${e.prim.tme} fge=${e.prim.fge} abe=${e.prim.abe} fst=${e.prim.fst} ctxt=${e.prim.ctxt} (${e.prim.raw})`);
      ...
      for (const ctx of r.contexts) {
        ...
        lines.push(`TEX0: TBP0=0x${ctx.tex0.tbp0.toString(16)} TBW=${ctx.tex0.tbw} PSM=0x${ctx.tex0.psm.toString(16)} TW=${ctx.tex0.tw} TH=${ctx.tex0.th} TCC=${ctx.tex0.tcc} (${ctx.tex0.raw})`);
        lines.push(`CLAMP: WMS=${ctx.clamp.wms} WMT=${ctx.clamp.wmt} U=[${ctx.clamp.minu},${ctx.clamp.maxu}] V=[${ctx.clamp.minv},${ctx.clamp.maxv}]`);
        lines.push(`SCISSOR: X=[${ctx.scissor.scax0},${ctx.scissor.scax1}] Y=[${ctx.scissor.scay0},${ctx.scissor.scay1}]`);
        lines.push(`ALPHA: A=${ctx.alpha.a} B=${ctx.alpha.b} C=${ctx.alpha.c} D=${ctx.alpha.d} FIX=0x${ctx.alpha.fix.toString(16)}`);
        lines.push(`TEST: ATE=${ctx.test.ate} ATST=${ctx.test.atst} AREF=${ctx.test.aref} ZTE=${ctx.test.zte} ZTST=${ctx.test.ztst} DATE=${ctx.test.date}`);
        lines.push(`FRAME: FBP=0x${ctx.frame.fbp.toString(16)} FBW=${ctx.frame.fbw} PSM=0x${ctx.frame.psm.toString(16)} FBMSK=${ctx.frame.fbmsk}`);
        lines.push(`ZBUF: ZBP=0x${ctx.zbuf.zbp.toString(16)} PSM=0x${ctx.zbuf.psm.toString(16)} ZMSK=${ctx.zbuf.zmsk}`);
...
server.tool('pcsx2_continue', 'Resume execution until breakpoint or halt.',
  ...
  async () => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try { const r = await debugServer!.resume(); return { content: [{ type: 'text' as const, text: `Resumed (paused=${r.paused}). Use pcsx2_pause to stop.` }] }; }
    catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);

server.tool('pcsx2_resume_and_wait', 'Resume and block until the VM pauses again (breakpoint/watchpoint hit) or the timeout elapses. Unlike pcsx2_continue, this tells you exactly when/whether a breakpoint fired instead of requiring a polling loop with pcsx2_status. Requires DebugServer.',
...
server.tool('pcsx2_pause', 'Pause/halt the emulator. Returns current PC.',
  ...
  async () => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try { const r = await debugServer!.pause(); return { content: [{ type: 'text' as const, text: `Paused at PC=${r.pc}` }] }; }
    catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);
...
// ==========================================================
server.tool('pcsx2_resolve_address', 'Resolve an address to its symbol name + offset, if known. Works even without a running VM if debug symbols are loaded. Requires DebugServer.',
  { address: z.string(), cpu: z.enum(['ee', 'iop']).default('ee') },
  async ({ address, cpu }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
      ...
      return { content: [{ type: 'text' as const, text: `${r.name}${offsetStr} (base ${r.address}, size ${r.size})` }] };
    } catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);

server.tool('pcsx2_find_symbol', 'Look up the address of a named symbol/function. Requires DebugServer.',
  { name: z.string(), cpu: z.enum(['ee', 'iop']).default('ee') },
  async ({ name, cpu }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
...
server.tool('pcsx2_get_function_info', 'Get the function containing an address: name, boundaries, no-return flag. Requires DebugServer.',
  { address: z.string(), cpu: z.enum(['ee', 'iop']).default('ee') },
  async ({ address, cpu }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
      ...
      return { content: [{ type: 'text' as const, text: `${r.name} @ ${r.address} (size ${r.size}${r.is_no_return ? ', no-return' : ''})` }] };
    } catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);

server.tool('pcsx2_list_functions', 'List known functions, optionally restricted to an address range. Symbols come from loaded debug info or pcsx2_scan_functions. Requires DebugServer.',
  { start: z.string().optional(), end: z.string().optional(), limit: z.number().min(1).max(2000).default(200), cpu: z.enum(['ee', 'iop']).default('ee') },
  async ({ start, end, limit, cpu }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
      const { total_count, functions } = await debugServer!.listFunctions({ start, end, limit, cpu });
      if (functions.length === 0) return { content: [{ type: 'text' as const, text: 'No functions found in that range.' }] };
      const lines = functions.map(f => `${f.address}  ${f.name.padEnd(32)} size=${f.size}`);
      return { content: [{ type: 'text' as const, text: `${total_count} total, showing ${functions.length}:\n${lines.join('\n')}` }] };
    } catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);
...
//  TOOL: Code Analysis
// ==========================================================
server.tool('pcsx2_decode_instruction', 'Structured decode of one instruction: branch target/condition, data-access address, etc. - everything MIPSAnalyst knows without re-deriving it from the disasm string. Requires DebugServer.',
  { address: z.string(), cpu: z.enum(['ee', 'iop']).default('ee') },
  async ({ address, cpu }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
      ...
      if (info.is_branch) lines.push(`Branch${info.is_conditional ? ' (conditional' + (info.condition_met ? ', TAKEN' : ', not taken') + ')' : ''} -> ${info.branch_target}${info.is_linked_branch ? ' (linked)' : ''}`);
...
server.tool('pcsx2_assemble', 'Assemble one MIPS instruction (e.g. "addiu a0, a0, 4") into its encoded opcode. Pair with pcsx2_write_memory to patch code live. Requires DebugServer.',
  { address: z.string(), instruction: z.string(), cpu: z.enum(['ee', 'iop']).default('ee') },
  async ({ address, instruction, cpu }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
...
server.tool('pcsx2_scan_functions', 'Heuristically detect function boundaries in a range with no debug info (typical for retail games). Adds them to the symbol database, visible afterward via pcsx2_list_functions/pcsx2_resolve_address. Requires DebugServer.',
  { start: z.string(), end: z.string(), generate_hashes: z.boolean().default(true), cpu: z.enum(['ee', 'iop']).default('ee') },
  async ({ start, end, generate_hashes, cpu }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
      const r = await debugServer!.scanFunctions(start, end, { generateHashes: generate_hashes, cpu });
      return { content: [{ type: 'text' as const, text: `Functions: ${r.functions_before} -> ${r.functions_after} (+${r.functions_added})` }] };
    } catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);
...
const memComparisonEnum = z.enum(['equals', 'not_equals', 'greater', 'less', 'greater_equal', 'less_equal', 'changed', 'unchanged', 'increased', 'decreased']);

server.tool('pcsx2_search_memory_start', 'Start a memory value scan over an address range (Cheat-Engine style). Omit comparison/value to capture all addresses as a baseline for a later "changed"/"increased" scan. Requires DebugServer.',
  { start: z.string(), end: z.string().optional(), type: memValueTypeEnum.default('u32'), comparison: memComparisonEnum.optional(), value: z.number().optional(), cpu: z.enum(['ee', 'iop']).default('ee') },
  async ({ start, end, type, comparison, value, cpu }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
...
server.tool('pcsx2_search_memory_next', 'Narrow the current memory scan\'s candidates by re-checking each address against a comparison (e.g. after the in-game value changed). Requires DebugServer.',
  { comparison: memComparisonEnum, value: z.number().optional() },
  async ({ comparison, value }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
...
server.tool('pcsx2_search_memory_results', 'List the current memory scan\'s candidate addresses and values. Requires DebugServer.',
  ...
  async ({ limit }) => {
    ...
    try {
      ...
      return { content: [{ type: 'text' as const, text: `${r.candidateCount} total (type ${r.type}), showing ${r.results.length}:\n${lines.join('\n')}` }] };
    } catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);

server.tool('pcsx2_search_memory_reset', 'Clear the current memory scan session. Requires DebugServer.',
  {},
  async () => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try { await debugServer!.searchMemoryReset(); return { content: [{ type: 'text' as const, text: 'Scan session cleared.' }] }; }
    catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);
...
// ==========================================================
server.tool('pcsx2_get_game_info', 'Get the currently loaded game\'s title, serial, ELF path, and CRC. Requires DebugServer.',
  {},
  async () => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
      ...
      return { content: [{ type: 'text' as const, text: `${info.title}\nSerial: ${info.serial}  CRC: ${info.crc}\nELF: ${info.elf}` }] };
    } catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);

server.tool('pcsx2_reset_vm', 'Cold-reset the running VM (e.g. to restart a repro from scratch). Requires DebugServer.',
  {},
  async () => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try { const r = await debugServer!.resetVM(); return { content: [{ type: 'text' as const, text: `VM reset. paused=${r.paused}` }] }; }
    catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);

server.tool('pcsx2_frame_advance', 'Advance emulation by N frames (e.g. while paused, to step forward a controlled amount). Blocks until the VM has actually re-paused. Requires DebugServer.',
...
server.tool('pcsx2_save_state_slot', 'Save state to a numbered slot (0-9). Requires DebugServer.',
  { slot: z.number().min(0).max(9) },
  async ({ slot }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try { const r = await debugServer!.saveStateSlot(slot); return { content: [{ type: 'text' as const, text: `Saved to slot ${slot}. paused=${r.paused}` }] }; }
    catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);

server.tool('pcsx2_load_state_slot', 'Load state from a numbered slot (0-9). Requires DebugServer.',
  { slot: z.number().min(0).max(9), backup: z.boolean().default(false) },
  async ({ slot, backup }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try { const r = await debugServer!.loadStateSlot(slot, backup); return { content: [{ type: 'text' as const, text: `Loaded slot ${slot}. paused=${r.paused} (note: pause state reflects what the VM was doing before this call, not what it was when the state was saved)` }] }; }
    catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);

server.tool('pcsx2_change_disc', 'Change/load a disc image (ISO path) into the running VM. Requires DebugServer.',
  { path: z.string(), source: z.enum(['iso', 'disc', 'nodisc']).default('iso') },
  async ({ path: discPath, source }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try { const r = await debugServer!.changeDisc(discPath, source); return { content: [{ type: 'text' as const, text: `Changed disc to ${discPath}. paused=${r.paused}` }] }; }
    catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);
...
server.tool('pcsx2_get_gamedb_info', 'PCSX2\'s own compatibility metadata for the running game: compatibility rating, EE/VU0/VU1 rounding & clamp modes (useful if float results look "off" - could be a known rounding quirk, not a bug in your analysis), known gameFixes PCSX2 already applies, and PCSX2\'s own embedded compatibility patch text for this exact CRC if one exists (often names real addresses). Requires DebugServer.',
  ...
  async () => {
    ...
    try {
      ...
      if (!info.alive) return { content: [{ type: 'text' as const, text: 'No VM running.' }] };
      if (!info.found) return { content: [{ type: 'text' as const, text: 'This game is not in PCSX2\'s GameDB (no known compatibility data).' }] };
      const lines = [
      ...
      if (info.game_fixes && info.game_fixes.length > 0) lines.push(`Game fixes applied: ${info.game_fixes.join(', ')}`);
...
server.tool('pcsx2_list_game_patches', 'List community-authored pnach patches/cheats for the running game (CRC-matched). Name + description + author only, no raw addresses - but descriptions often hint at what\'s interesting and where (e.g. "Infinite Health", "Unlock All Levels"). Requires DebugServer.',
  { cheats: z.boolean().default(true) },
  async ({ cheats }) => {
    if (!hasDebug()) return { content: [{ type: 'text' as const, text: 'Error: DebugServer not connected.' }], isError: true };
    try {
      const { unlabelledCount, patches } = await debugServer!.listGamePatches(cheats);
      if (patches.length === 0) return { content: [{ type: 'text' as const, text: 'No patches/cheats found for this game.' }] };
      const lines = patches.map(p => `${p.name}${p.author ? ` (by ${p.author})` : ''}${p.description ? `\n  ${p.description}` : ''}`);
      const note = unlabelledCount > 0 ? `\n(${unlabelledCount} additional unlabelled patch line(s) not shown individually)` : '';
      return { content: [{ type: 'text' as const, text: lines.join('\n') + note }] };
    } catch (e: any) { return { content: [{ type: 'text' as const, text: `Error: ${e.message}` }], isError: true }; }
  }
);
